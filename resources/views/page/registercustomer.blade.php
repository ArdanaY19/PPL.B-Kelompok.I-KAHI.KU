        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="alert alert-primary" role="alert">
                    {{ session('sukses') }}<a href="/login" class="alert-link"> Silahkan Login</a>
                </div>
                @endif
                <div class="card-header"><h3>{{ __('Register Customer') }}</h3></div>

                <div class="card-body">
                    <form method="POST" action="/postregistercustomer" enctype="multipart/form-data" name="form" onsubmit="return validateForm()">
                        @csrf
                        <div class="form-group row">
                            <label for="name" class="col-md-4 col-form-label text-md-right">{{ __('Nama') }}</label>

                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" value="{{ old('nama') }}" autocomplete="nama" autofocus>
...
<script type="text/javascript">
    function validateForm() {
        ...
        var d = document.forms["form"]["password-confirm"].value;
        var e = document.forms["form"]["email"].value;
        var f = document.forms["form"]["alamat"].value;
        var g = document.forms["form"]["no_hp"].value;
        var h = document.forms["form"]["tanggal_lahir"].value;
        var i = document.forms["form"]["foto"].value;
